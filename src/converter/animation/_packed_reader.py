...
class OdinPackedReader(OdinAnimationReader):
    def __init__(self, gltf, animation: dict):
        ...
        self.packed: dict = animation.get("packed")
        self.nodes: list[dict] = self.packed.get("nodes")
        self.stride = 12

        # Normalized transform values
        self.normalized_transform_data = gltf.decode_accessor(
            self.packed.get("dataAccessor")
        )

        # Base transform values
        self.node_base_data = gltf.decode_accessor(self.packed.get("nodeAccessor"))
        ...
        self.keyframe_mapping: list[int] = []
        self.node_base_data_offset = 0

    def process_node(self, node_index: int):
        node = self.nodes[node_index]
        flags = self.flags[node_index]
        total_frame_count: int = node.get("frameCount")
        self.data_size: int = node.get("dataSize")
        self.node_base_data_offset: int = node_index * self.stride
    ...
    def denormalize_transforms(
        ...
        bTranslation: Sequence[float | int],
        bRotation: Sequence[float | int],
        bScale: Sequence[float | int],  # Base transform
        nTranslation: np.ndarray,
        nRotation: np.ndarray,
        nScale: np.ndarray,  # Delta transforms
    ) -> tuple[list[np.ndarray], list[np.ndarray], list[np.ndarray]]:
    ...
    def read(self) -> None:
        self.keyframe_mapping: list[int] = [
            node.get("frameCount") for node in self.nodes
        ]
    ...
    @override
    def get_frame_data(
        self, node_index: int, frame_index: int
